...
/* DMI Board names of HP laptops support manual fan control */
...
static bool is_manual_fan_control_board(void)
{
	...
	return match_string(manual_fan_control_boards,
...
static int hp_wmi_fan_speed_set(int fan1_speed, int fan2_speed)
{
	...
	// TODO: explain
	// TODO: use short instead of long
	fan1_speed = fan1_speed / 100;
	fan2_speed = fan2_speed / 100;

	u8 fan_speed[2] = { fan1_speed, fan2_speed };
	int ret;
			      ...
			      u32 attr, int channel, long val)
{
	...
	case hwmon_fan:
		if (is_manual_fan_control_board()) {
			pr_info("Manual fan control board detected\n");
			return hp_wmi_fan_speed_set(val, val);
		} else {
			pr_info("Manual fan control board not detected\n");
			return -EINVAL;
		}
